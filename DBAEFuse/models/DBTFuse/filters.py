...
def gaussian_kernel(kernel_size: int = 5, sigma: float | None = None):
    ...
    weight = torch.exp(-torch.sum((xy_grid - center) ** 2.0, dim=-1) / (2 * sigma**2))
...
class GaussianFilter(nn.Module):
    def __init__(self, kernel_size: int = 5, sigma: float | None = None, channels: int = 1):
        super().__init__()
        ...
        self.filter = nn.Conv2d(
            ...
            bias=False,
            padding=pad,
            padding_mode="replicate",  # reflect
            groups=channels,
        )
...
class BilateralFilter(nn.Module):
    def __init__(self, kernel_size: int = 5, sigma_space: float | None = None, sigma_density: float | None = None):
        super().__init__()
